...
export async function sendPostsScroll(createdAt) {
  const result = await db.query(`
    SELECT 
      posts.id AS "postId",
      posts.content AS content,
...
export async function sendRepostsScroll(createdAt) {
  const result = await db.query(`
    SELECT 
      posts.id AS "postId",
      posts.content AS content,
...
export async function getUserPostsScroll(id, lastPost) {
  const promise = db.query(
    `SELECT 
    posts.id AS "postId",
    posts.content AS content,
...
export async function getUserRepostsScroll(id, lastPost) {
  return db.query(
    `
    SELECT 
    posts.id AS "postId",
